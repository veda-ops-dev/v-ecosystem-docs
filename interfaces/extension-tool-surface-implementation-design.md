# Extension Tool-Surface Implementation Design

## Purpose

This document defines the mechanical implementation design for the extension tool surface in the V Ecosystem.

It exists to answer:

```text
How should the extension construct, filter, refresh, and enforce the active tool surface so the runtime only receives the tools it is currently entitled to use under system, scope, governance, and delegation constraints?
```
...
- the current governance/action posture
...
- execution adapter / dispatcher binding
...
- current governance/action posture
...
- delegated/runtime role posture
...
Current-system clarity is one of the strongest anti-drift constraints in the extension model.
...
Before the active tool surface is published to the runtime / system-init layer, the runtime should run a final sanity filter.
...
- current governance/action posture still allows the tool
...
- whether the active surface is in a restricted / review-sensitive posture
...
- when designing absent-affordance enforcement in the extension runtime
