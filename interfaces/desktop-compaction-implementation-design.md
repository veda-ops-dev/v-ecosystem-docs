# Desktop Compaction Implementation Design

## Purpose

This document defines the mechanical implementation design for runtime compaction in the V Ecosystem desktop application.

It exists to answer:

```text
How should the desktop runtime perform compaction mechanically so long-running sessions remain usable without violating the already-established continuity, governance, state, and operator-visibility doctrine?
```
...
Before any compaction stage begins, the runtime must evaluate the current session against the canonical compaction blocking rules already defined in `desktop-memory-and-continuity-model.md`.
...
The full desktop-wide consequences of a compaction event — per-surface behavior, LLM behavior, blocking posture, and sequencing — are defined in `desktop-invalidation-and-refresh-matrix.md` (EVENT-14 — Meaningful Compaction Event). This section defines the runtime-layer refresh mechanics that support those consequences.
...
- when implementing token-pressure handling in the desktop runtime
...
- `desktop-state-and-context-model.md`
- `desktop-system-init-and-tool-surface-model.md`
- `desktop-invalidation-and-refresh-matrix.md` — canonical event-to-consequence mapping; EVENT-14 defines the full desktop consequences of a compaction event that this doc's runtime mechanics must support
- `../governance/agent-operating-doctrine.md`
