...
```text
What counts as current state in the extension, what must always be visible to the operator and to the LLM, how are workflow stage, decision context, evidence basis, and session integrity represented, how is stale or incomplete context surfaced, how is cross-system referenced context shown without implying ownership transfer, and what must never be treated as canonical state?
```
...
- context visibility rules for the operator and the LLM
...
The extension must never allow either the operator or the LLM to be uncertain about what state is actually loaded, what system is active, what workflow stage is current, what governing decisions apply, what evidence basis is loaded, or what the LLM does not currently know.
...
The extension must track and surface six categories of state. These are not all the same kind of state — some are operator-set, some are loaded from governed records, some are session-computed. All six must be visible and current.
...
Pending approvals, unresolved return triggers, stale readiness states, and unresolved decision conflicts from prior sessions or current session activity that require operator attention before proceeding.
...
- the basis for which system surface commands and buttons are available
...
### Referenced Systems
Referenced systems are the other systems that provide bounded context into the current session. They provide read-only, non-canonical, visibly-attributed context.
...
- whether any decision has a staleness marker (see Stale Context section)
...
### Visible on demand but explicitly accessible (context strip)
...
The LLM must not reference a specific governing decision, evidence record, or workflow stage as though it is loaded context unless that item appears in the current context strip. If it is not shown as loaded, the LLM must not assert it as though it is current.
...
Stale context does not block Class A actions but must block Class B and Class C approval gates until refreshed or acknowledged.
...
The following events must invalidate the relevant context category and require a reload before the extension proceeds normally:
...
When a new session starts, all six state categories must be loaded fresh from governed records. Prior session cache is not canonical even if it appears current.
...
- session history is not canonical state and must not be cited as governing basis
- the extension will surface what the LLM does not know — the LLM must honor those boundaries rather than paper over them

If the LLM treats the richness of its loaded context as authorization to act, or cites prior session outputs as governing evidence, this state model is being violated.
